...
worker_code <- c(
  ...
  "        }",
  "",
  "        # Re-binarize y when OD has binary outcome (logistic data + norm/pmm method)",
  "        if (y_is_binary && method != 'cart') {",
  "          p_hat_y <- mean(od$y, na.rm = TRUE)",
  "          thr_y   <- quantile(sd$y, probs = 1 - p_hat_y, na.rm = TRUE)",
  "          sd$y    <- as.numeric(sd$y >= thr_y)",
  "        }",
  "",
  "        sd$iter <- it",
