...
Given several factors, is there any combination of these factors which conducts to a more similar clustering result according to the real data?

-   Generation of real data with clusters with 2SD and 6SD separation (correlation degree)
-   SPECKS bias correction 
-   Change in the calculation of optimal k for clustering (computational efficiency)
-   Add indicator of the number of cases where clustReal = clustSynt (the whole analyse is based on this restriction)
...
# (1). Real data generation with clusters separation
This section tests different libraries for the creation of a synthetic data generation function, which could enable the assignment of different factors combinations.

## library(mvtnorm)
For p>=5 and k>=3, a separation of 10SD may be necessary for a clear separation between clusters. And for a less clear clusters, a distance of 6SD is fine.

Problem: Correlation becomes uncontrollable as we increase the number of parameters and clusters in the data.
```{r}
#### Manual generation ####
library(mvtnorm)
...
### Function to obtain the cluster centroids
```{r}
# function 
...
## library(clusterGeneration) -> NOT SUITABLE
The clusters are generated, but there is no way to control the correlation between the variables.
-   Eigen method: random correlation
-   Unifcorrmax method: using the value given by the formula, but the desired correlation can't be achieved either.
...
## library(MixSim) -> NOT SUITABLE
Clusters in data are generated with the desired separations, but the correlation is still uncontrolable. 
sph=TRUE forces a spherical covariance matrix, i.e. zero correlation.
sph=FALSE and controlling the value of ecc give some different correlation, but there is no pattern that allows control.
...
To reduce computational cost, instead of using NbClust, which calculates 30 metrics to decide the k for k-means, we'll use the silhouette index as criterion to find the optimal k.
```{r}
...
From the data frame containing all possible combinations of the factors taken into account, add a new variable indicating the number of attempts until generating the X (e.g. 300) that satisfy clustReal = clustSynt.
...
write.csv(real.data, file = "../data/1/original_data.csv", row.names = FALSE)
write.csv(synt.data, file = "../data/1/synthetic_data.csv", row.names = FALSE)
...
# ==========================================
# 2. Define Functions (From your original code)
# ==========================================

centroid.generation <- function(k, p, separation) {
  centroids <- matrix(NA, nrow = k, ncol = p)
  centroids[1, ] <- rep(0, p)  # First centroid at origin
  
  for (i in 2:k) {
    tries <- 0
    repeat {
      vector <- rnorm(p)
      vector <- vector / sqrt(sum(vector^2))  # Normalize vector
      vector <- vector * separation           # Scale to separation distance
...
synt.data.generation <- function(real.data, m, method){
    # We use proper = TRUE to ensure full synthetic data generation logic
    syn.obj <- syn(real.data, method = method, m = m, print.flag = FALSE)
    return(syn.obj$syn)
}
...
# Parameters (Dataset 1 Configuration)
set.seed(123)
N <- 250
p <- 2
rho <- 0.0
...
cat("Generating Real Data...\n")
rdata_obj <- data.generation(N = N, p = p, rho = rho, separation = separation, k = k)
real.data <- rdata_obj$Data

# 2. Generate Synthetic Data
cat("Generating Synthetic Data (CART)...\n")
synt.data <- synt.data.generation(real.data, m = 1, method = method)

# 3. Export to CSV
output_dir <- "../data/2"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE) # recursive=TRUE creates parent folders if needed
}

# Note: We remove row names to keep the CSV clean for Python
write.csv(real.data, file = "../data/2/original_data.csv", row.names = FALSE)
write.csv(synt.data, file = "../data/2/synthetic_data.csv", row.names = FALSE)

cat("Success! Files saved to:\n")
cat(" - data/2/original_data.csv\n")
cat(" - data/2/synthetic_data.csv\n")
```