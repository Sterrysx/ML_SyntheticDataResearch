...
We see in the previous part that the behaviours for both methods are really similar, except for the silhouette value, which will be analyzed more deeply in this section.
```{r}
# cart
result <- result_cart_50_allmetrics
...
-   Method is not really relevant, do have some effect on silhouette. "Norm" method require more computation cost.


# (3). Gini for unbalances cluster
In the graphic representation for all analyzed metrics, we saw for the gini metric, the associate boxplot is always situated under 0. A possible explanation for this behavior is that the real data set is created based on balanced clusters. In this section unbalanced dataset groups will be create. 
